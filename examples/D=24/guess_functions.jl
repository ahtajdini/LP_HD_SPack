...
function smartGuess(oldroots, newN)
    ...
    for i in range(1,newN)
        rNew[i] = BigFloat(oldroots[1])*(1-BigFloat(newN)/BigFloat(n))+ BigFloat(newN)/BigFloat(n)*rRef(1 + BigFloat((i-1)*(n-1))/BigFloat((newN-1)))
    end
    return rNew
end

#newN is in practice =len(oldroots)+1
function smartGuess2(oldroots, newN)
    n= length(oldroots)
    rRef = linear_interpolation(range(1,n),oldroots,extrapolation_bc=Line())
    rNew = zeros(BigFloat,(newN,))
    for i in range(1,newN)
...
function smartGuess2ndorder(oldroots1, oldroots2,newN)
    ...
    print(rRef.(range(1,newN)))
    rRef2 = fit(Polynomial,range(1,n2),oldroots2)
    rNew = zeros(newN)
    for i in range(1,newN)
        first= (newN/n2)*rRef2(1+(i-1)*(n2-1)/(newN-1))
        second = (newN-n2)/(n2-n1)*( rRef2(1+(i-1)*(n2-1)/(newN-1))
         - rRef1(1+(i-1)*(n1-1)/(newN-1)))
        rNew[i] = first+second
